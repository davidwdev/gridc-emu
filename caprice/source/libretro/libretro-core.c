...
static bool disk_set_eject_state(bool ejected)
{
	...
	if ( dc->eject_state != ejected )
	{
		...
		if ( dc->eject_state )
		{
			log_cb(RETRO_LOG_INFO, "Ejected Disk(%d)\n", dc->index+1 );

			detach_disk(0);
		}
		else
		{
			log_cb(RETRO_LOG_INFO, "Disk(%d) re-inserted: %s\n", dc->index+1, dc->files[dc->index] );
...
static bool disk_replace_image_index( unsigned index, const struct retro_game_info *info )
{
	...
	// tidy up existing media in this slot, if present.
	if ( dc->files[ index ] )
	{
		free( dc->files[ index ] );
		dc->files[ index ] = NULL;
	...
	if ( info )
	{
		// We're replacing with new media.
	...
	else
	{
		...
		if ( index < dc->count )
		{
			//log_cb( RETRO_LOG_INFO, "remove slot %d. dc->count = %d. dc->index = %d\n", index, dc->count+1, dc->index );

			// We're removing media, not at the end of the list. Move everything up by one.
			for ( unsigned i = index; i < dc->count; ++i )
			{
				//log_cb( RETRO_LOG_INFO, "copy slot %d -> slot %d\n", i+1, i );
				dc->files[ i ] = dc->files[ i + 1 ];
			}

			// delete the (now duplicated) entry in the last slot.
			//log_cb( RETRO_LOG_INFO, "delete final slot %d\n", dc->count );
			if ( dc->files[ dc->count ] )
			{
				free( dc->files[ dc->count ] );
				dc->files[ dc->count ] = NULL;
			}
		}

		// ensure the selected index is correct.
		if ( dc->index > 0 && dc->index >= dc->count )
			--dc->index;

		log_cb( RETRO_LOG_INFO, "Disk in slot (%d) removed. Eject state = %d\n", index+1, dc->eject_state );
	}
